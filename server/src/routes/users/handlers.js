...
const filterByUser = (user, prop) => doc => doc.requests.some(x => x[prop] === user);
...
const parsePending = user => arr => ({
    incoming: arr.filter(filterByUser(user, 'tutor')),
    outgoing: arr.filter(filterByUser(user, 'student')),
});
